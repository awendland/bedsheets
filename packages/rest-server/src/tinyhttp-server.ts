...
import { fold, getOrElse, isLeft } from "fp-ts/lib/Either"
...
export async function runServer({
  ...
  const sheetsApi = await SheetsDAL.auth.getSheetsClient(process.env as any)
...
export const decodeParamsAs: <T>(
  decoder: Decoder<unknown, T>
) => SyncHandler<Request & { params: T }, Response> = (decoder) => (
  req,
  res,
  next
) => {
